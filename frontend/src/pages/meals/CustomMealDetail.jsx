...
const CustomMealDetail = () => {
    ...
    const fetchCustomMeal = async () => {
      try {
        ...
        const ingredientsResponse = await axios.get(
          `${API_URL}/meals/custom-meals/${id}/ingredients/`,
          {
            headers: token ? { Authorization: `Token ${token}` } : {},
          }
        );
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (!restaurantId && customMeal.base_meal) {
        try {
          const token = localStorage.getItem("token");
          const baseMealResponse = await axios.get(
            `${API_URL}/meals/meals/${customMeal.base_meal}/`,
            {
              headers: token ? { Authorization: `Token ${token}` } : {},
            }
          );
          ...
          restaurantName =
            baseMealResponse.data.restaurant_name || "Restaurant";
        } catch (error) {
      ...
      if (!restaurantId) {
        try {
          ...
          alert(
            "Could not determine the restaurant for this custom meal. Please try again."
          );
          return;
        }
      }
  ...
  if (!customMeal) {
    return <div className="alert alert-warning">Custom meal not found.</div>;
  }
    ...
    <div className="custom-meal-detail-page">
      <div className="breadcrumb" style={{ marginBottom: "1rem" }}>
        ...
        <Link
          to="/profile"
          onClick={() =>
            document.querySelector(".menu-tab:nth-child(3)").click()
          }
        >
          {" "}
          My Custom Meals
      ...
      >
        <div className="meal-image-container">
          <img
            src={
              customMeal.image ||
              "https://via.placeholder.com/500x400?text=Custom+Meal"
            }
            alt={customMeal.name}
            style={{
              width: "100%",
        ...
        <div className="meal-details">
          ...
          <p style={{ color: "#666", marginBottom: "1rem" }}>
            Custom Meal • Created on{" "}
            {new Date(customMeal.created_at).toLocaleDateString()}
          </p>
            ...
            <div className="meal-ingredients" style={{ marginBottom: "2rem" }}>
              ...
              <ul style={{ listStyleType: "disc", paddingLeft: "1.5rem" }}>
                {ingredients.map((item) => (
                  <li key={item.id}>
                    {item.ingredient_details.name} ({item.quantity}{" "}
                    {item.ingredient_details.unit})
                    <span>
                      {" "}
                      - $
                      {(
                        item.ingredient_details.price_per_unit * item.quantity
                      ).toFixed(2)}
                    </span>
                  </li>
                ))}
