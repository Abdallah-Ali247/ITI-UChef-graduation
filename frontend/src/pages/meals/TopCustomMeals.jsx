  ...
  const handleAddToCart = async (meal) => {
    ...
    try {
      ...
      const ingredientsResponse = await axios.get(
        `${
          import.meta.env.VITE_API_URL || "http://localhost:8000"
        }/api/meals/custom-meals/${meal.id}/ingredients/`
      );
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {topMeals.length === 0 ? (
        <div className="text-center py-12">
          <h2 className="text-xl font-semibold mb-4">
            No rated custom meals yet
          </h2>
          <p className="mb-6">
            Be the first to create and share a custom meal!
          </p>
            ...
            topMeals.map((meal, index) => (
              <div key={meal.id} className="card">
                ...
                <img
                  src={
                    meal.image ||
                    "https://via.placeholder.com/300x200?text=Custom+Meal"
                  }
                  alt={meal.name}
                  className="card-img"
                />
                  ...
                  >
                    <div className="average-rating">
                      <span className="rating-number">
                        {meal.avg_rating ? meal.avg_rating.toFixed(1) : "0"}
                      </span>
                      {renderStars(meal.avg_rating)}
                      <span className="review-count">
                        ({meal.review_count}{" "}
                        {meal.review_count === 1 ? "review" : "reviews"})
                      </span>
                    </div>
                  </div>
      ...
      >
        ...
        <p style={{ marginBottom: "1.5rem" }}>
          Create your own custom meal and share it with the community to see if
          it can reach the top!
        </p>
