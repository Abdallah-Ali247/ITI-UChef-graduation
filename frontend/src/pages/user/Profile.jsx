...
const Profile = () => {
  ...
  const { customMeals, loading: mealsLoading } = useSelector(
    (state) => state.meals
  );
  ...
  const handleAddToCart = async (meal) => {
    ...
    try {
      ...
      const mealResponse = await axios.get(
        `${API_URL}/meals/custom-meals/${meal.id}/`,
        {
      ...
      if (mealDetails.base_meal) {
        ...
        try {
          const baseMealResponse = await axios.get(
            `${API_URL}/meals/meals/${mealDetails.base_meal}/`,
            {
              headers: token ? { Authorization: `Token ${token}` } : {},
            }
          );
      ...
      if (!restaurantId) {
        try {
          ...
          throw new Error(
            "Could not determine the restaurant for this custom meal"
          );
        }
      }
      ...
      const ingredientsResponse = await axios.get(
        `${API_URL}/meals/custom-meals/${meal.id}/ingredients/`,
        {
          headers: token ? { Authorization: `Token ${token}` } : {},
        }
      );
      ...
      console.log("Adding custom meal to cart with restaurant:", {
        restaurantId,
        restaurantName,
      });
                  ...
                  {user.profile && (
                    <>
                      ...
                      >
                        ...
                        <p>
                          {user.profile.favorite_cuisine || "Not specified"}
                        </p>
                      </div>
                    </>
                  )}
                  ...
                  <div key={meal.id} className="card">
                    <div className="card-body">
                      ...
                      {meal.base_meal_details && (
                        <p>
                          <small>Based on: {meal.base_meal_details.name}</small>
                        </p>
                      )}
                      ...
                      <p>
                        <span
                          style={{
                            ...
                            backgroundColor: meal.is_public
                              ? "#d4edda"
                              : "#f8f9fa",
