...
const ReviewForm = ({ type, itemId, onReviewSubmitted, existingReview = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingReview) {
        ...
        response = await axios.put(`${endpoint}${existingReview.id}/`, reviewData, { headers });
      } else {
        // Create new review
      ...
      if (error.response?.status === 400 && error.response?.data?.non_field_errors?.[0]?.includes('unique')) {
        setError('You have already reviewed this item. You can edit your existing review instead.');
        ...
        setError(error.response?.data?.detail || error.message || 'Failed to submit review');
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="rating-input">
            {[1, 2, 3, 4, 5].map(star => (
              <label key={star} className="star-label">
                ...
                />
                <span className={`star ${formData.rating >= star ? 'filled' : ''}`}>★</span>
              </label>
            ))}
          </div>
        </div>
